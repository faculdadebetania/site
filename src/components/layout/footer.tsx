    ...
    <footer className="container text-white !pb-0">
      <section className="grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Link
          href="e-mec"
          target="_blank"
          className="flex justify-between items-center gap-4 rounded-lg bg-sky-950/50 p-4 w-64"
        >
          <h3 className="font-medium text-secondary break-keep">Consulte aqui o cadastro da Instituição no Sistema e-MEC</h3>
          <Image
            priority
            width={100}
            height={144}
            src="/imagens/e-mec.png"
            alt="logo"
            quality={100}
      ...
      <section className="text-zinc-700 flex flex-col md:flex-row items-center justify-between gap-2 stext-center px-2 pb-4 lg:px-8 lg:pb-6">
        <div>© {new Date().getFullYear()} Faculdade Teológica Betânia.</div>
        <div className="flex gap-4 justify-center">
          <Button className="text-neutral-400 hover:opacity-50 hover:bg-neutral-100 h-10 w-10 hover:text-primary transition">
